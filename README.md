...
面向 RustDesk 自建服务器（`hbbs`/`hbbr`）的 Go 语言安装工具。它将原 `main.sh` 的全部逻辑重构为跨平台的可执行程序，提供交互式部署、下载进度显示、systemd 服务管理及无 systemd 环境的守护进程启动。

- 交互式部署：支持公网 IP、域名（DDNS）与内网静态 IP 选择
- 端口固定：`hbbs` 21116、`hbbr` 21117（不支持自定义）
- 下载进度：实时显示百分比、速度（MB/s）与 ETAß
- 安装检测：已安装时提供“覆盖安装 / 卸载 / 取消”菜单
...
- 功能覆盖：包含 RustDesk 专业版所有功能，配套管理与监控，支持自定义端口，lucky反向代理，NAT机器部署，内网穿透部署ß
